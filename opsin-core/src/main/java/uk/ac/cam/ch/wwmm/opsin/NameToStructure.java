...
public class NameToStructure {
	...
	public OpsinResult parseChemicalName(String name, NameToStructureConfig n2sConfig) {
		...
		try {
			...
			return new OpsinResult(null, OPSIN_RESULT_STATUS.FAILURE, message, name, null, null);
		}
		...
		for(Element parse : parses) {
			try {
				...
				String outputXml = outputParse.toXML();
				
				if (state.getWarnings().isEmpty()) {
					return new OpsinResult(frag, OPSIN_RESULT_STATUS.SUCCESS, "", name, parseXml, outputXml);
				}
		...
		if (fragGeneratedWithWarning != null) {
			return new OpsinResult(fragGeneratedWithWarning, OPSIN_RESULT_STATUS.WARNING, warnings, name, parseXmlForWarning, outputXmlForWarning);
		}
		return new OpsinResult(null, OPSIN_RESULT_STATUS.FAILURE, reasonForFailure, name, null, null);
	}
