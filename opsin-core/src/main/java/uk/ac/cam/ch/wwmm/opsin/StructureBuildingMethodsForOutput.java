 ...
 *
 */
class StructureBuildingMethodsForOutput{
	private static final Logger LOG = LogManager.getLogger(StructureBuildingMethodsForOutput.class);
	private static final Pattern matchCompoundLocant =Pattern.compile("[\\[\\(\\{](\\d+[a-z]?'*)[\\]\\)\\}]");

	private StructureBuildingMethodsForOutput() {}
		...
		for(int i=heteroatoms.size() -1;i >= 0;i--) {
			...
			if(locant != null) {
				...
				if (heteroatom.getElement() == atomToBeReplaced.getElement() && heteroatom.getCharge() == atomToBeReplaced.getCharge()){
					// MolLangData: if the thisFrag's subType is a hantzsch-widman ring, we should not throw an exception, we can directly continue, since the heteroatom is already replaced,
					// the tokenEl is maintained for the final output not for a new fragment
					if (thisFrag.getSubType().equals(HANTZSCHWIDMAN_SUBTYPE_VAL)) {
						continue;
					}
	...
	static void resolveUnLocantedFeatures(BuildState state, Element subOrRoot) throws StructureBuildingException {
		...
		List<Element> children = subOrRoot.getChildElements();
		for (Element currentEl : children) {
			// MolLangData: since we do not detach the locanted elements in the xml for the output, so if the element is a locanted element, we should skip
			if (currentEl.getAttributeValue(LOCANT_ATR) != null) {
				continue;
			}
			String elName = currentEl.getName();
			if (elName.equals(UNSATURATOR_EL)) {
