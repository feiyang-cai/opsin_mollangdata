 ...
 * This is a separate handler for StructureBuilderForOutput that does not modify the original parse.
 * @author dl387
 *
 */
class StereochemistryHandlerForOutput {
	
	private static final Logger LOG = LogManager.getLogger(StereochemistryHandlerForOutput.class);
	...
	StereochemistryHandlerForOutput(BuildState state, Map<Atom, StereoCentre> atomStereoCentreMap, Map<Bond, StereoBond> bondStereoBondMap) {
		this.state = state;
	...
	private void matchStereochemistryToAtomsAndBonds(Element stereoChemistryEl) throws StructureBuildingException, StereochemistryException {
		...
		if (stereoChemistryType.equals(R_OR_S_TYPE_VAL)){
			...
			applyGlobalRacOrRelFlags(stereoChemistryEl, StereoGroupType.Rac);
		}
	 ...
	 * @param groupType the stereo group Rac(emic) or Rel(ative).
	 * @throws StructureBuildingException
	 * @throws StereochemistryException
	 */
	private void applyGlobalRacOrRelFlags(Element stereoChemistryEl, StereoGroupType groupType) throws StructureBuildingException, StereochemistryException {
		...
		if (undefinedStereo.size() > 0) {
			...
			undefinedStereo.get(0).setStereoGroup(new StereoGroup(groupType));
	...
	private void assignStereoCentre(Element stereoChemistryEl) throws StructureBuildingException, StereochemistryException {
		...
		for (Fragment fragment : possibleFragments) {
			if (attemptAssignmentOfStereoCentreToFragment(fragment, rOrS, locant, grpType)) {
				// MolLangData: we copy the stereoChemistryEl before the fragment that is assigned the stereo chemistry
				Element fragmentTokenEl = fragment.getTokenEl();
				if (fragmentTokenEl != null) {
					Element stereoChemistryElCopy = stereoChemistryEl.copy();
					OpsinTools.insertBefore(fragmentTokenEl, stereoChemistryElCopy);
				}
				return;
			}
		}
		...
		if (possibleWordParent.getName().equals(WORD_EL) && possibleWordParent.getChild(0).equals(parentSubBracketOrRoot)){
			...
			for (Element word : words) {
				List<Element> possibleGroups = OpsinTools.getDescendantElementsWithTagName(word, GROUP_EL);
				for (int i = possibleGroups.size()-1; i >=0; i--) {
					Fragment fragment = possibleGroups.get(i).getFrag();
					if (attemptAssignmentOfStereoCentreToFragment(fragment, rOrS, locant, grpType)) {
						// MolLangData: we copy the stereoChemistryEl before the fragment that is assigned the stereo chemistry
						Element fragmentTokenEl = fragment.getTokenEl();
						if (fragmentTokenEl != null) {
	...
	private void assignStereoBond(Element stereoChemistryEl) throws StructureBuildingException, StereochemistryException {
		...
		for (Fragment fragment : possibleFragments) {
			if (attemptAssignmentOfStereoBondToFragment(fragment, eOrZ, locant, isCisTrans)) {
				// MolLangData: we copy the stereoChemistryEl before the fragment that is assigned the stereo chemistry
				// MolLangData: for Cis/Trans, it only applies the situation where is only one double bond, so no locants are needed
				// MolLangData: for E/Z, for more than one double bond, the locants should be explicitly specified, so the locants are already in the stereoChemistryEl, no need to add them again
				Element fragmentTokenEl = fragment.getTokenEl();
				if (fragmentTokenEl != null) {
					Element stereoChemistryElCopy = stereoChemistryEl.copy();
					OpsinTools.insertBefore(fragmentTokenEl, stereoChemistryElCopy);
	...
	private boolean assignCisTransOnRing(Element stereoChemistryEl) throws StructureBuildingException {
		...
		for (Fragment fragment : possibleFragments) {
			String[] locants = attemptAssignmentOfCisTransRingStereoToFragment(fragment, stereoChemistryEl);
			if (locants != null){
				// MolLangData: we copy the stereoChemistryEl before the fragment that is assigned the stereo chemistry
				Element fragmentTokenEl = fragment.getTokenEl();
				if (fragmentTokenEl != null) {
					Element stereoChemistryElCopy = stereoChemistryEl.copy();
					// MolLangData: add the locants of a1 and a2 to the copied element
					if (locants.length == 2 && locants[0] != null && locants[1] != null) {
						stereoChemistryElCopy.addAttribute(new Attribute(LOCANT_ATR, locants[0] + "," + locants[1]));
					}
					OpsinTools.insertBefore(fragmentTokenEl, stereoChemistryElCopy);
				}
				return true;
	...
	private String[] attemptAssignmentOfCisTransRingStereoToFragment(Fragment fragment, Element stereoChemistryEl) throws StructureBuildingException {
		List<Atom> atomList = fragment.getAtomList();
		...
		if (chosenStereoAtoms.size() == 2) {
			...
			// MolLangData: return the locants of a1 and a2
			String locant1 = a1.getFirstLocant();
			String locant2 = a2.getFirstLocant();
			if (locant1 != null && locant2 != null) {
				return new String[]{locant1, locant2};
			}
			return new String[0]; // Return empty array to indicate success but no locants
		}
		return null;
	}
			...
			if (notExplicitlyDefinedStereoCentreMap.containsKey(potentialStereoAtom) && potentialStereoAtom.getBondCount() == 4) {
				...
				if (acidGroup != null && amineOrAlcohol != null && sideChain != null && hydrogen != null) {
					...
					if (dOrL.equals("l") || dOrL.equals("ls")) {
						...
						potentialStereoAtom.setAtomParity(atomRefs4, 1);
						potentialStereoAtom.setStereoGroup(new StereoGroup(StereoGroupType.Rac,++state.numRacGrps));
					} else{
